...
1. في [استوديو التعلم الآلي من Microsoft Azure](https://ml.azure.com?azure-portal=true)، قم بتوسيع الجزء الأيسر عن طريق تحديد أيقونة القائمة في أعلى يسار الشاشة. اعرض صفحة **المصمم** (ضمن **Authoring**)، وحدد **+** لإنشاء مسار جديد.

1. قم بتغيير اسم المسودة (**Pipeline-Created-on-date****) إلى **Auto Price Training**.
...
1. حدد **Component**. ابحث عن مجموعة البيانات **Automobile price data (Raw)** وضعها على اللوحة.
...
1. أغلق نافذة **DataOutput** بحيث يمكنك رؤية مجموعة البيانات على اللوحة على النحو التالي:
...
1. في جزء **Asset library** على اليسار، حدد **Component**، والذي يحتوي على مجموعة واسعة من الوحدات النمطية التي يمكنك استخدامها لتحويل البيانات وتدريب النموذج. يمكنك أيضا استخدام شريط البحث لتحديد موقع الوحدات بسرعة.
...
1. انقر نقرا مزدوجا فوق الوحدة **النمطية تنظيف البيانات المفقودة** ، وفي الجزء الموجود على اليمين، حدد **تحرير العمود**. ثم في نافذة **Columns to be cleaned**، حدد **With rules**، وفي قائمة **Include**، حدد **Column names**، وفي مربع أسماء الأعمدة، أدخل **bore** و**stroke** و**horsepower** هكذا:
...
1. في صفحة **Basics** حدد **Create new** وقم بتعيين اسم التجربة إلى **mslearn-auto-training** ثم حدد **Next** .
...
1. انتظر بضع دقائق حتى تنتهي عملية التشغيل. يمكنك التحقق من حالة الوظيفة عن طريق تحديد **Jobs** ضمن **Assets**. من هناك، حدد مهمة **Auto Price Training** . من هنا، يمكنك معرفة وقت اكتمال المهمة. بمجرد اكتمال المهمة، يتم الآن إعداد مجموعة البيانات لتدريب النموذج.

1. انتقل إلى القائمة اليسرى. ضمن **Authoring** حدد **المصمم**. ثم حدد *البنية الأساسية لبرنامج ربط العمليات التجارية للتدريب على السعر التلقائي* من قائمة **المسارات**.
...
1. تأكد من تحديد **القائمة اليسرى المصمم** وأنك قد عدت إلى البنية الأساسية **لبرنامج ربط العمليات التجارية للتدريب على السعر التلقائي**.
...
1. حدد **Configure & Submit**، وقم بتشغيل البنية الأساسية لبرنامج ربط العمليات التجارية باستخدام التجربة الحالية المسماة **mslearn-auto-training**.
...
1. سيستغرق تشغيل التجربة بضع دقائق لإكمالها. ارجع إلى صفحة **Jobs** وحدد أحدث تشغيل لوظيفة **Auto Price Training** .
...
1. يتضمن الإخراج الوارد من الوحدة النمطية **Score Model** جميع ميزات الإدخال وكذلك التسمية المتوقعة. لتعديل الإخراج كي يتضمن التنبؤ فقط:
    ...
    ```
>**ملاحظة**: قد يؤدي النسخ واللصق إلى إدخال مسافات في برنامج Python النصي الذي يجب ألا يكون هناك. تحقق مرة إضافية من عدم وجود مسافة قبل *الاستيراد* أو *التحد* أو *الإرجاع*. تأكد من وجود مسافات بادئة لعلامة تبويب واحدة قبل *scored_results* *scored_results.rename()*.

1. قم بتوصيل الإخراج من الوحدة النمطية **Score Model** بإدخال **Dataset1** (أقصى اليسار) من **Execute Python Script**.

1. قم بتوصيل إخراج **مجموعة بيانات النتيجة** (إلى اليسار) للوحدة **النمطية Execute Python Script** بالوحدة النمطية **Web Service Output** .
...
1. ارجع إلى صفحة **Jobs** وحدد أحدث تشغيل لمهمة **Auto Price Training** (المهمة المتعلقة *بتجربة الاستدلال التلقائي mslearn-auto-inference*).

1. عند اكتمال البنية الأساسية لبرنامج ربط العمليات التجارية، انقر بزر الماوس الأيمن فوق الوحدة النمطية **Execute Python Script** . حدد أيقونة **معاينة البيانات** ثم **مجموعة بيانات النتائج** لمشاهدة الأسعار المتوقعة للسيارات الثلاث في بيانات الإدخال.
