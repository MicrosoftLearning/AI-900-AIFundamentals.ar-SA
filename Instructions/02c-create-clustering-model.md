...
1. تغيير اسم المسودة (**Pipeline-Created-on-date***) إلى **Train Penguin Clustering**.
...
1. *حدد علامة التبويب ملف التعريف*، مع ملاحظة أنه يمكنك رؤية توزيعات الأعمدة المختلفة كم مدرجات تكرارية.
...
1. حدد **حفظ**، ثم أغلق **القائمة تحديد الأعمدة في مجموعة** البيانات للعودة إلى لوحة المصمم.

1. في **مكتبة** الأصول، ابحث عن الوحدة النمطية **Clean Missing Data** وضعها على اللوحة، أسفل **الوحدة النمطية Select columns in dataset** وقم بتوصيلها على النحو التالي:
...
1. انقر نقرا مزدوجا فوق الوحدة النمطية **Clean Missing Data** ، وفي جزء الإعدادات على اليمين، حدد **Edit column**. ثم في نافذة **Columns to be cleaned** حدد **With rules** وقم بتضمين **All columns**؛ على النحو التالي:
...
1. في **صفحة Basics** حدد **Create new** وضبط اسم التجربة على **mslearn-penguin-training** ثم حدد **Next** .
...
1. انتظر حتى تنتهي عملية التشغيل. قد يستغرق ذلك 5 دقائق أو أكثر. يمكنك التحقق من حالة المهمة عن طريق تحديد **Jobs** ضمن **Assets**. من هناك، حدد **وظيفة Train Penguin Clustering** .


## اعرض البيانات المحولة

1. عند اكتمال التشغيل، ستبدو الوحدات النمطية كما يلي:

    ![لقطة شاشة للوحدات بحالة مكتملة مع شريط أخضر على يسار كل وحدة.](media/create-clustering-model/normalize-complete.png)

1. انقر بزر الماوس الأيمن فوق الوحدة النمطية **Normalize Data** ، وحدد **Preview data**، ثم حدد **Transformed dataset** لعرض النتائج.
...
1. تجمّع خوارزمية *K-Means* العناصر في عدد أنظمة المجموعات التي تحددها - وهي قيمة يشار إليها باسم ***K***. حدد الوحدة **K-Means Clustering** وفي جزء الجانب الأيمن، قم بتعيين معلمة **Number of centroids** إلى **3**.
    ...
    > 5. الخطوات المتكررة ج. و د. حتى تستقر عمليات تخصيص نظام المجموعة أو يكتمل العدد المحدد من التكرارات.
...
1. حدد **Configure and Submit**، وقم بتشغيل المسار باستخدام التجربة الحالية المسماة **mslearn-penguin-training** على مجموعة الحوسبة الخاصة بك.

1. انتظر حتى تنتهي عملية التشغيل. قد يستغرق ذلك 5 دقائق أو أكثر. تحقق من حالة المهمة عن طريق تحديد **Jobs** ضمن **Assets**. من هناك، حدد أحدث **وظيفة Train Penguin Clustering** .
...
1. في **صفحة المصمم**، افتح **مسودة مسار Train Penguin Clustering**.
...
1. انقر بزر الماوس الأيمن فوق الوحدة النمطية Evaluate Model **، وحدد **Preview data**، ثم حدد **Evaluation results**.** راجع المقاييس في كل صف:
...
1. حدد موقع القائمة أعلى اللوحة وحدد **Create inference pipeline**. قد تحتاج إلى توسيع الشاشة إلى كامل والنقر على **أيقونة ...** في الزاوية العلوية اليسرى من الشاشة من أجل العثور على **إنشاء مسار** الاستدلال في القائمة.  
...
1. أعد تسمية المسار الجديد باسم **"Predict Penguin Clusters" **، ثم راجع المسار الجديد. تعد التحويلات ونموذج التجميع في البنية الأساسية لبرنامج ربط العمليات التجارية للتدريب جزءا من هذا المسار. سيتم استخدام النموذج المدرب لتسجيل البيانات الجديدة. يحتوي المسار أيضًا على web service output لإرجاع النتائج.
    ...
    - قم بإزالة **المكون Select Columns in Dataset** ، والذي أصبح الآن زائدا عن الحاجة.
    - ** الاتصالمكونات إدخال** خدمة ويب وإدخال **البيانات يدويا** (التي تمثل مدخلات للبيانات المراد تجميعها) إلى المكون الأول **تطبيق التحويل**.

    اتبع الخطوات المتبقية أدناه، باستخدام الصورة والمعلومات الموجودة أعلاه كمرجع في أثناء تعديل البينة الأساسية.

1. لا يتضمن المسار تلقائيًا مكون **Web Service Input** للنماذج التي تم إنشاؤها من مجموعات البيانات المخصصة. ابحث عن مكون **Web Service Input** من asset library وضعه في أعلى المسار.  الاتصال إخراج **مكون إدخال** خدمة ويب إلى *إدخال مجموعة* البيانات (على اليمين) لأول **مكون Apply Transformation** الموجود بالفعل على اللوحة.  
...
1. الاتصال إخراج **أدخل الوحدة النمطية *Data Manually** إلى إدخال مجموعة* البيانات (على اليمين) للوحدة النمطية Apply Transformation** الأولى**.
...
1. عند انتهاء المسار، انقر بزر الماوس الأيمن فوق **الوحدة النمطية Assign Data to Clusters** ، وحدد **Preview data** وحدد **Results dataset** لمشاهدة تعيينات ومقاييس نظام المجموعة المتوقعة لملاحظات البطريق الثلاث في بيانات الإدخال.
...
1. أثناء وجودك في **صفحة Predict Penguin Clusters** inference run، حدد **Deploy** في شريط القوائم العلوي.
...
1. لعرض حالة النشر، قم بتوسيع الجزء الأيمن عن طريق تحديد أيقونة القائمة في أعلى يسار الشاشة. اعرض صفحة **Endpoints** (ضمن **Assets**) وحدد **predict-penguin-clusters**. عند انتهاء التوزيع، ستتغير **Deployment state** إلى **Healthy**.
