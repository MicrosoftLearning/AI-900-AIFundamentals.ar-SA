...
1. في [استوديو التعلم الآلي من Microsoft Azure](https://ml.azure.com?azure-portal=true)، قم بتوسيع الجزء الأيمن عن طريق تحديد أيقونة القائمة في أعلى يسار الشاشة. اعرض صفحة **المصمم** (ضمن **Authoring**)، وحدد **+** لإنشاء مسار جديد.

1. غير اسم المسودة (**Pipeline-Created-on-date***) إلى **Train Penguin Clustering**.
...
1. ارجع إلى البنية الأساسية لبرنامج ربط العمليات التجارية عن طريق **Designer** في القائمة اليسرى. في صفحة **المصمم**، حدد مسودة مسار **Train Penguin Clustering**.
...
1. حدد **Data**، وابحث عن مجموعة **بيانات penguin-data** وضعها على اللوحة.

1. انقر بزر الماوس الأيمن (Ctrl+النقر فوق Mac) فوق مجموعة البيانات **penguin-data** على اللوحة، وانقر فوق **Preview data**.

1. حدد علامة التبويب *ملف التعريف* ، مع ملاحظة أنه يمكنك رؤية توزيعات الأعمدة المختلفة كمدرج تكراري.
...
1. أغلق صفحة **DataOutput** حتى تتمكن من رؤية مجموعة البيانات على لوحة البنية الأساسية لبرنامج ربط العمليات التجارية.
...
1. حدد **حفظ**، ثم أغلق قائمة **تحديد الأعمدة في مجموعة البيانات** للعودة إلى لوحة المصمم.

1. في **مكتبة الأصول**، ابحث عن وحدة **Clean Missing Data** وضعها على اللوحة، أسفل الوحدة **النمطية Select columns in dataset** وقم بتوصيلها على النحو التالي:
...
1. انقر نقرا مزدوجا فوق الوحدة **النمطية تنظيف البيانات المفقودة** ، وفي جزء الإعدادات على اليمين، حدد **تحرير العمود**. ثم في نافذة **Columns to be cleaned** حدد **With rules** وقم بتضمين **All columns**؛ على النحو التالي:
...
1. حدد **حفظ** ، ثم أغلق إعدادات الوحدة **النمطية Normalize Data** للعودة إلى لوحة المصمم.

## تشغيل المسار

لتطبيق تحويلات البيانات الخاصة بك، تحتاج إلى تشغيل البنية الأساسية كتجربة.

1. حدد **Configure & Submit** في أعلى الصفحة لفتح حوار **مهمة إعداد البنية الأساسية** لبرنامج ربط العمليات التجارية.

1. في صفحة **Basics** حدد **Create new** وقم بتعيين اسم التجربة إلى **mslearn-penguin-training** ثم حدد **Next** .

1. في صفحة **Inputs & outputs** حدد **Next** دون إجراء أي تغييرات.
...
1. حدد **Next** لمراجعة مهمة البنية الأساسية لبرنامج ربط العمليات التجارية ثم حدد **Submit** لتشغيل مسار التدريب.

1. انتظر حتى تنتهي عملية التشغيل. قد يستغرق ذلك 5 دقائق أو أكثر. يمكنك التحقق من حالة الوظيفة عن طريق تحديد **Jobs** ضمن **Assets**. من هناك، حدد مهمة **Train Penguin Clustering** .


## عرض البيانات المحولة

1. عند اكتمال التشغيل، ستبدو الوحدات كما يلي:

    ![لقطة شاشة للوحدات بحالة مكتملة مع شريط أخضر على يسار كل وحدة.](media/create-clustering-model/normalize-complete.png)

1. انقر بزر الماوس الأيمن فوق الوحدة **Normalize Data** ، وحدد **Preview data**، ثم حدد **Transformed dataset** لعرض النتائج.
...
1. أغلق صفحة **Transformed_dataset** للعودة إلى تشغيل البنية الأساسية لبرنامج ربط العمليات التجارية.
...
1. ارجع إلى صفحة **المصمم** وافتح مسودة مسار **Train Penguin Clustering**.
...
1. سوف يستخدم النموذج الذي نقوم بتدريبه الميزات لتجميع البيانات في مجموعات بيانات، لذلك نحن بحاجة لتدريب النموذج باستخدام خوارزمية *نظام المجموعات*. في **مكتبة الأصول**، ابحث عن وحدة **K-Means Clustering** وضعها في اللوحة، إلى يسار الوحدة النمطية **Split data** وفوق الوحدة **النمطية Train Clustering Model** . ثم قم بتوصيل المخرج الخاص بها مع مدخل **Untrained model** للوحدة النمطية **Train Clustering Model** (الموجود على اليسار).
...
1. تجمّع خوارزمية *K-Means* العناصر في عدد أنظمة المجموعات التي تحددها - وهي قيمة يشار إليها باسم ***K***. حدد الوحدة **K-Means Clustering** وفي جزء الجانب الأيمن، قم بتعيين معلمة **Number of centroids** إلى **3**.
    ...
    > 5. الخطوات المتكررة ج. و د. حتى تستقر تخصيصات نظام المجموعة أو يكتمل العدد المحدد من التكرارات.
...
1. حدد **Configure & Submit**، وقم بتشغيل البنية الأساسية لبرنامج ربط العمليات التجارية باستخدام التجربة الحالية المسماة **mslearn-penguin-training** على مجموعة الحوسبة الخاصة بك.

1. انتظر حتى تنتهي عملية التشغيل. قد يستغرق ذلك 5 دقائق أو أكثر. تحقق من حالة المهمة عن طريق تحديد **Jobs** ضمن **Assets**. من هناك، حدد أحدث مهمة **Train Penguin Clustering** .

1. عند انتهاء تشغيل التجربة، انقر بزر الماوس الأيمن فوق الوحدة **النمطية Assign Data to Clusters** ، وحدد **Preview data** ثم حدد **Results dataset** لعرض النتائج.
...
1. أغلق **مرئيات Results_dataset** للعودة إلى تشغيل البنية الأساسية لبرنامج ربط العمليات التجارية.
...
1. في صفحة **المصمم**، افتح مسودة البنية الأساسية لبرنامج ربط العمليات التجارية **Train Penguin Clustering**.
...
1. انتظر حتى انتهاء تشغيل التجربة. للتحقق من حالتها، انتقل إلى صفحة **Jobs** وحدد أحدث مهمة **Train Penguin Clustering** .

1. انقر بزر الماوس الأيمن فوق الوحدة النمطية **تقييم النموذج** ، وحدد **معاينة البيانات**، ثم حدد **نتائج التقييم**. راجع المقاييس في كل صف:
    - **متوسط المسافة إلى مركز آخر**
    - **متوسط المسافة إلى مركز نظام المجموعة**
    - **عدد النقاط**
    - **المسافة القصوى إلى مركز نظام المجموعة**

1. أغلق علامة التبويب **Evaluation_results** .
...
1. حدد موقع القائمة أعلى اللوحة وحدد **Create inference pipeline**. قد تحتاج إلى توسيع الشاشة إلى وضع ملء وانقر فوق أيقونة **...** في الزاوية العلوية اليمنى من الشاشة للعثور على **إنشاء مسار الاستدلال** في القائمة.  
...
1. في القائمة المنسدلة **Create inference pipeline** حدد **Real-time inference pipeline**. وبعد بضع ثوان، سيتم فتح إصدار جديد من المسار الخاص بك باسم **"Train Penguin Clustering-real time inference"**.
...
1. لا يتضمن المسار تلقائيًا مكون **Web Service Input** للنماذج التي تم إنشاؤها من مجموعات البيانات المخصصة. ابحث عن مكون **Web Service Input** من asset library وضعه في أعلى المسار.  قم بتوصيل إخراج مكون **إدخال خدمة الويب** بإدخال *مجموعة البيانات* (إلى اليمين) لمكون **Apply Transformation** الأول الموجود بالفعل على اللوحة.  
...
1. قم بتوصيل إخراج الوحدة النمطية **Enter Data Manually** بإدخال *مجموعة البيانات* (على اليمين) للوحدة النمطية **Apply Transformation** الأولى.
...
1. انتقل إلى **Jobs** وحدد أحدث وظيفة **Predict Penguin Clusters** باستخدام اسم تجربة  **mslearn-penguin-inference** .

1. عند انتهاء البنية الأساسية لبرنامج ربط العمليات التجارية، انقر بزر الماوس الأيمن فوق **الوحدة النمطية Assign Data to Clusters** ، وحدد **Preview data** وحدد **Results dataset** لمشاهدة تعيينات المجموعة المتوقعة والمقاييس لملاحظات البطريق الثلاثة في بيانات الإدخال.
...
في هذا التمرين، سوف تنشر خدمة الويب على مثيل حاوية Azure Container Instance (ACI). يتم إنشاء هذا النوع من الحساب بصورة ديناميكية، ويعد مفيداً في التطوير والاختبار. بالنسبة للإنتاج، يجب عليك إنشاء *نظام مجموعة استدلال* يوفر نظام مجموعة Azure Kubernetes Service (AKS) والتي توفر قابلية توسع وأمان بصورة أفضل.

1. بينما في صفحة Predict **Penguin Clusters** inference run، حدد **Deploy** في شريط القوائم العلوي.
...
1. حدد **Deploy a new real-time endpoint** واستخدم الإعدادات التالية:
