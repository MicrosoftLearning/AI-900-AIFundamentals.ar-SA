...
1. في [استوديو التعلم الآلي من Microsoft Azure](https://ml.azure.com?azure-portal=true)، قم بتوسيع الجزء الأيسر عن طريق تحديد أيقونة القائمة في أعلى يسار الشاشة. حدد صفحة **البيانات** (ضمن **الأصول**). تحتوي صفحة "Data" على ملفات بيانات أو جداول معينة تخطط للعمل معها في Azure ML. يمكنك إنشاء مجموعات بيانات من هذه الصفحة أيضًا.
...
## إنشاء البنية الأساسية لبرنامج ربط العمليات التجارية في المصمم وتحميل البيانات إلى اللوحة

للبدء مع مصمم التعلم الآلي من Azure، يجب أولاً إنشاء مسار وإضافة مجموعة البيانات التي تريد العمل معها.

1. في [استوديو التعلم الآلي من Microsoft Azure](https://ml.azure.com?azure-portal=true)، في الجزء الأيمن، حدد عنصر **المصمم** (ضمن **Authoring**)، ثم حدد **+** لإنشاء مسار جديد.

1. تغيير اسم المسودة من **Pipeline-Created-on-*date*** إلى **Diabetes Training**.
...
1. حدِّد ⁧**⁩Data⁧**⁩. ابحث عن مجموعة بيانات **diabetes-data** وضعها في اللوحة.

1. انقر بزر الماوس الأيمن (Ctrl+click على جهاز Mac) على مجموعة بيانات **diabetes-data** على اللوحة، وحدد **Preview data**.
...
1. أغلق علامة التبويب **DataOutput** حتى تتمكن من رؤية مجموعة البيانات على اللوحة على النحو التالي:
...
1. ابحث عن الوحدة **النمطية Select Columns in Dataset** وضعها على اللوحة أسفل مجموعة بيانات **diabetes-data** . ثم قم بتوصيل الإخراج من الجزء السفلي لمجموعة بيانات **diabetes-data** بالإدخال الموجود في الجزء العلوي من وحدة **Select Columns in Dataset**.
...
1. ابحث عن الوحدة **النمطية Normalize Data** وضعها على اللوحة أسفل الوحدة **النمطية Select Columns in Dataset** . ثم قم بتوصيل الإخراج من الجزء السفلي لوحدة **Select Columns in Dataset** بالإدخال الموجود في الجزء العلوي من وحدة **Normalize Data**، مثل ما يلي:
...
1. في صفحة **Basics** حدد **Create new** وقم بتعيين اسم التجربة إلى **mslearn-diabetes-training** ثم حدد **Next** .
...
1. حدد **Review + Submit** لمراجعة مهمة البنية الأساسية لبرنامج ربط العمليات التجارية ثم حدد **Submit** لتشغيل مسار التدريب.

1. انتظر بضع دقائق حتى تنتهي عملية التشغيل. يمكنك التحقق من حالة الوظيفة عن طريق تحديد **Jobs** ضمن **Assets**. من هناك، حدد تجربة **mslearn-diabetes-training** ثم وظيفة **Diabetes Training** .

## عرض البيانات المحولة

عند اكتمال التشغيل، يتم الآن إعداد مجموعة البيانات لتدريب النموذج.

1. انقر بزر الماوس الأيمن (Ctrl+click على جهاز Mac) فوق الوحدة **Normalize Data** على اللوحة، وحدد **Preview data**. حدد **Transformed dataset**.
...
1. ارجع إلى صفحة **المصمم** وحدد مسار **Diabetes Training**.
...
1. حدد **Configure & Submit**، وقم بتشغيل البنية الأساسية لبرنامج ربط العمليات التجارية باستخدام التجربة الحالية المسماة **mslearn-diabetes-training**.

1. انتظر حتى انتهاء تشغيل التجربة. قد يستغرق ذلك 5 دقائق أو أكثر.

1. تحقق من حالة المهمة عن طريق تحديد **Jobs** ضمن **Assets**. من هناك، حدد تجربة **mslearn-diabetes-training** ثم حدد أحدث وظيفة **لتدريب مرض السكري** .

1. في علامة التبويب الجديدة، انقر بزر الماوس الأيمن فوق (Ctrl+click on a Mac) الوحدة النمطية **Score Model** على اللوحة، وحدد **Preview data** ثم حدد **Scored dataset** لعرض النتائج.
...
1. ارجع إلى **المصمم** وافتح مسار **Diabetes Training** الذي أنشأته.
...
1. في علامة التبويب الجديدة، انقر بزر الماوس الأيمن (Ctrl+click على جهاز Mac) الوحدة النمطية **Evaluate Model** على اللوحة، وحدد **Preview data** ثم حدد **Evaluation results** لعرض مقاييس الأداء. يمكن أن تساعد هذه المقاييس علماء البيانات في تقييم مدى جودة تنبؤ النموذج بناءً على بيانات التحقق من الصحة.
...
1. في القائمة المنسدلة **Create inference pipeline** ، حدد **Real-time inference pipeline**. بعد بضع ثوان، سيتم فتح نسخة جديدة من البنية الأساسية لديك باسم **Diabetes Training-real time inference**.
...
1. قم بتوصيل الوحدة النمطية **الجديدة Enter Data Manually** بنفس إدخال **مجموعة البيانات** للوحدة النمطية **Select Columns in Dataset** مثل **Web Service Input**.
...
1. قم بتوصيل الإخراج من الوحدة النمطية **Score Model** بإدخال *Dataset1* (الأكثر يسارا) من **Execute Python Script**، وقم بتوصيل إخراج *مجموعة بيانات النتيجة* (إلى اليسار) للوحدة النمطية **Execute Python Script** **بإخراج خدمة الويب**.
...
1. ارجع إلى علامة التبويب  **Jobs** . من هناك، حدد تجربة **mslearn-diabetes-inference** ثم حدد وظيفة **Predict Diabetes** .

1. عند اكتمال البنية الأساسية لبرنامج ربط العمليات التجارية، حدد الوحدة النمطية **Execute Python Script** . حدد **Preview data** وحدد **Result dataset** لعرض الأوصاف والاحتمالات المتوقعة لملاحظات المرضى الثلاثة في بيانات الإدخال.
...
1. في الجزء العلوي من نافذة وظيفة **Predict Diabetes** ، حدد **Deploy**.
...
1. في **Set up real-time endpoint** حدد **Deploy new real-time endpoint** واستخدم الإعدادات التالية:
    - **Name**: predict-diabetes
    - **الوصف**: تصنيف مرض السكري
    - **نوع الحساب**: Azure Container Instance

1. حدد **Deploy** وانتظر حتى يتم نشر خدمة الويب - قد يستغرق ذلك عدة دقائق.
