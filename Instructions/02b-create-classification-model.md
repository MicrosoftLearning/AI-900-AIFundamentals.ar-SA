...
1. في [Azure التعلم الآلي studio](https://ml.azure.com?azure-portal=true)، قم بتوسيع الجزء الأيمن عن طريق تحديد أيقونة القائمة في أعلى يسار الشاشة. **حدد صفحة البيانات** (ضمن **الأصول**). تحتوي صفحة "Data" على ملفات بيانات أو جداول معينة تخطط للعمل معها في Azure ML. يمكنك إنشاء مجموعات بيانات من هذه الصفحة أيضًا.
...
1. في [Azure التعلم الآلي studio](https://ml.azure.com?azure-portal=true)، في الجزء الأيمن حدد **عنصر المصمم** (ضمن **Authoring**)، ثم حدد **+** لإنشاء مسار جديد.
...
1. في **صفحة Basics** حدد **Create new** وضبط اسم التجربة على **mslearn-diabetes-training** ثم حدد **Next** .
...
1. حدد **Review + Submit** لمراجعة مهمة المسار ثم حدد **Submit** لتشغيل مسار التدريب.

1. انتظر بضع دقائق حتى تنتهي عملية التشغيل. يمكنك التحقق من حالة المهمة عن طريق تحديد **Jobs** ضمن **Assets**. من هناك، حدد **تجربة mslearn-diabetes-training** ثم **وظيفة Diabetes Training** .

## اعرض البيانات المحولة

عند اكتمال التشغيل، يتم الآن إعداد مجموعة البيانات لتدريب النموذج.

1. انقر بزر الماوس الأيمن (Ctrl+click على جهاز Mac) فوق **الوحدة النمطية Normalize Data** على اللوحة، وحدد **Preview data**. حدد **Transformed dataset**.
...
1. حدد **Configure and Submit**، وقم بتشغيل المسار باستخدام التجربة الحالية المسماة **mslearn-diabetes-training**.
...
1. في علامة التبويب الجديدة، انقر بزر الماوس الأيمن (Ctrl+click على جهاز Mac) **على الوحدة النمطية Score Model** على اللوحة، وحدد **Preview data** ثم حدد **Scored dataset** لعرض النتائج.
...
1. لا يتضمن المسار تلقائيًا مكون **Web Service Input** للنماذج التي تم إنشاؤها من مجموعات البيانات المخصصة. ابحث عن مكون **Web Service Input** من asset library وضعه في أعلى المسار. الاتصال إخراج **مكون إدخال** خدمة ويب إلى **مكون Select Columns in Dataset** الموجود بالفعل على اللوحة.
...
1. الاتصال الإخراج من **** تسجيل نموذج الوحدة النمطية *إلى إدخال Dataset1* (أقصى اليسار) من **تنفيذ برنامج Python النصي**، وتوصيل *إخراج مجموعة* بيانات النتيجة (يسار) الوحدة **النمطية تنفيذ البرنامج النصي** Python إلى **إخراج** خدمة ويب.
...
1. ارجع إلى  **علامة التبويب المهام** . من هناك، حدد **تجربة mslearn-diabetes-inference** ثم حدد **وظيفة Predict Diabetes** .

1. عند اكتمال المسار، حدد الوحدة النمطية **Execute Python Script** . حدد **Preview data** وحدد **Result dataset** لعرض الأوصاف والاحتمالات المتوقعة لملاحظات المرضى الثلاثة في بيانات الإدخال.
...
1. في أعلى **نافذة وظيفة Predict Diabetes** ، حدد **Deploy**.
...
1. حدد **Deploy** وانتظر حتى يتم نشر خدمة الويب - قد يستغرق هذا عدة دقائق.
