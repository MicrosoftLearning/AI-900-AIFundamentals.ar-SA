...
توفر خدمة **Custom Vision** في Microsoft Azure حلا مستندا إلى السحابة لإنشاء نماذج مخصصة للكشف عن الكائنات ونشرها. في Azure، يمكنك استخدام خدمة Custom Vision لتدريب نموذج الكشف عن الكائنات استنادا إلى الصور الموجودة. هناك عنصران لإنشاء حل الكشف عن العناصر. أولا، يجب عليك تدريب نموذج للكشف عن موقع وفئة الكائنات باستخدام الصور المسماة. بعد ذلك، عندما يتم تدريب النموذج، يجب عليك نشره كخدمة يمكن أن تستهلكها التطبيقات.
...
> **ملاحظة** لا يتوفر كل مورد في كل منطقة. سواء قمت بإنشاء مورد خدمات Custom Vision أو Azure الذكاء الاصطناعي، يمكن استخدام الموارد التي تم إنشاؤها في [مناطق معينة](https://azure.microsoft.com/global-infrastructure/services/?products=cognitive-services) فقط للوصول إلى خدمات Custom Vision. من أجل البساطة، يتم تحديد المنطقة مسبقاً لك في إرشادات التكوين أدناه.
...
1. في مدخل Custom Vision، في مشروع الكشف عن عنصر **Traffic Safety** ، حدد **Add images** وقم بتحميل جميع الصور في المجلد المستخرج.
...
1. اضغط باستمرار على الماوس فوق أي عنصر (دراج أو مشاة) في الصورة حتى يتم عرض منطقة تم اكتشافها تلقائيا. ثم حدد الكائن، وإذا لزم الأمر، قم بتغيير حجم المنطقة المحيطة به. بدلاً من ذلك، يمكنك ببساطة السحب حول الكائن لإنشاء منطقة.

    عند تحديد الكائن بإحكام داخل المنطقة المستطيلة، أدخل العلامة المناسبة للكائن (*الراكب* أو *المشاة*) واستخدم زر **منطقة العلامة** (**+**) لإضافة العلامة إلى المشروع.

    ![لقطة شاشة لصورة مع منطقة ذات علامة في مربع الحوار Image Detaol.](media/create-object-detection-solution/tag-image.png)

1. استخدم الارتباط **التالي** (**>)** على اليمين للانتقال إلى الصورة التالية، ووضع علامة على عناصرها. ثم استمر في العمل من خلال مجموعة الصور بأكملها، ووضع علامات على كل دراج ومشاة.

    أثناء وضع علامة على الصور، لاحظ ما يلي:

    - تحتوي بعض الصور على كائنات متعددة، يحتمل أن تكون من أنواع مختلفة. وضع علامة على كل منها، حتى لو تداخلت.
...
2. عند اكتمال التدريب، راجع مقاييس أداء *الدقة* *والاستدعاء* *وmAP* - تقيس هذه المقاييس جودة التنبؤ لنموذج الكشف عن العناصر، ويجب أن تكون جميعها عالية بشكل معقول.

3. اضبط **عتبة الاحتمال** على اليسار، وقم بزيادتها من 50٪ إلى 90٪ ولاحظ التأثير على مقاييس الأداء. يحدد هذا الإعداد قيمة الاحتمال التي يجب أن يفي بها كل تقييم علامة أو يتجاوزها ليتم حسابها كتوقع.
...
1. انقر فوق **&#128504; Publish** لنشر النموذج المدربين مع الإعدادات التالية:
    - **اسم النموذج**: أمان نسبة استخدام الشبكة
